...
import os
from color_encoder.correction import add_y0combine
...
def efficient_generalized_steps(self,x, seq, model, b, missing,H_funcs, y_0, sketch , filename, sigma_0, etaB, etaA, etaC, cls_fn=None, classes=None):
    with torch.no_grad():
        ...
        for i, j in tqdm(zip(reversed(seq), reversed(seq_next))):
            ...
            V_t_x = H_funcs.Vt(xt_mod,sketch)
            SVt_x = (V_t_x * Sigma)[:, :U_t_y.shape[1]]
            V_t_x0 = H_funcs.Vt(x0_t,torch.zeros_like(x0_t))
            ...
            Vt_xt_mod_next = V_t_x0 + sigma_tilde_nextC * H_funcs.Vt(et,torch.zeros_like(et)) + std_nextC * torch.randn_like(V_t_x0)
            ...
            comb_name = 'exp/datasets/murals/3/'+filename
            xt_next = add_y0combine(xt_next ,missing, comb_name , step, device=xt.device)
