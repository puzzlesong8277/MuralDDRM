...
def get_image_filename(directory):
    ...
    image_filenames = [f for f in os.listdir(directory) if f.lower().endswith(image_extensions)] 
...
def get_dataset(args, config):
    ...
    if config.data.dataset == "CELEBA":
        ...
        if config.data.subset_2k:
            ...
            test_dataset =ImageDataset(os.path.join(args.exp, 'datasets', 'murals', '0'),#'22_10'
                     os.path.join(args.exp, 'txt/murals_test_2k_147.txt'),
                     image_size=config.data.image_size,
                     normalize=False) 
        if config.data.sketch:
            dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'sketch', '0'),#'contour_style''results_22_10','anime_style'
                     os.path.join(args.exp, 'txt/murals_test_2k_147.txt'),
                     image_size=config.data.image_size,
                     normalize=False)
        ...
        if config.data.subset_1k:
            ...
            dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'imagenet', 'imagenet'),
                     os.path.join(args.exp, 'imagenet_val_1k_147.txt'),
                     image_size=config.data.image_size,
                     normalize=False)
            test_dataset = dataset
